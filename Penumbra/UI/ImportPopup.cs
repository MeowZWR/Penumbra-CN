...
public sealed class ImportPopup : Window, INotificationAwareMessage
{
    ...
    public override void Draw()
    {
        ...
        terminate |= import.State is ImporterState.Done
            ? Im.Button("关闭"u8, Im.ContentRegion.Available with { Y = 0} )
    ...
    void IMessage.OnNotificationActions(INotificationDrawArgs args)
    {
        ...
        {
            if (Im.Button("打开报告"u8, Im.ContentRegion.Available with { Y = 0 }))
            {
                _openPopup = true;
                _notification?.DismissNow();
            }
        ...
        {
            if (Im.Button("显示详情"u8, new Vector2((Im.ContentRegion.Available.X - Im.GetStyle().ItemSpacing.X) * 0.5f, 0.0f)))
            {
                _openPopup = true;
                _notification?.DismissNow();
            }
