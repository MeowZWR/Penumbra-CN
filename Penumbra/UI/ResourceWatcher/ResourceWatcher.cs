...
public sealed class ResourceWatcher : IDisposable, ITab<TabType>
{
    ...
    public void DrawContent()
    {
        ...
        if (Im.Checkbox("启用"u8, _config.ResourceLoggerEnabled))
        ...
        Im.Line.Same();
        if (Im.Checkbox("仅存储匹配项"u8, _config.ResourceLoggerStoreOnlyMatching))
            _config.ResourceLoggerStoreOnlyMatching ^= true;

        Im.Line.Same();
        if (Im.Checkbox("写入日志"u8, _config.ResourceLoggerWriteToLog))
    ...
    private void DrawFilterInput()
        => _filter.DrawFilter("如果路径与此正则表达式匹配..."u8, Im.ContentRegion.Available);

    private void DrawMaxEntries()
    {
        Im.Item.SetNextWidthScaled(80);
        if (ImEx.InputOnDeactivation.Scalar("最大记录数"u8, _config.ResourceLoggerMaxEntries, out var newValue))
        ...
        if (_config.ResourceLoggerMaxEntries is not DefaultMaxEntries && Im.Item.Hovered())
            Im.Tooltip.Set("Ctrl + 右键点击重置为默认的条数：500."u8);
