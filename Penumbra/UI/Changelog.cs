...
public class PenumbraChangelog : IUiService
{
    ...
    private static void Add1_5_0_0(Changelog log)
        => log.NextVersion("版本 1.5.0.0")
            .RegisterImportant("更新以支持游戏版本 7.30 和 Dalamud API13，该版本使用了新的 GUI 后端。某些功能可能无法正常工作。如果您遇到任何问题，请告知我。")
            .RegisterEntry("添加了使用两种顶点颜色方案导出模型的支持（感谢 zeroeightysix！）。")
            .RegisterEntry("可能改善了导出模型时创建的基础颜色纹理的颜色准确性（感谢 zeroeightysix！）。")
            .RegisterEntry("由于崩溃问题，禁用了使用 characterstockings 着色器的材质的透明度启用功能（感谢 zeroeightysix！）。")
            .RegisterEntry("修复了模型输入/输出和无效切线的一些问题（感谢 PassiveModding！）")
            .RegisterEntry("更改了在使用模组标准化器与组合群组时默认目录名称的行为。")
            .RegisterEntry("向 HTTP API 添加了跳转到特定模组的功能。")
            .RegisterEntry("修复了角色声音模组的问题（1.4.0.6）。")
            .RegisterHighlight("添加了对第一个切换之外的饰品的 IMC 切换属性支持（1.4.0.5）。")
            .RegisterEntry("修复了在槽位间交换物品时模型中的一些槽位特定属性和形态（1.4.0.5）。")
            .RegisterEntry("为屏幕显示标签页和类似功能添加了人类皮肤材质处理（感谢 Ny！）（1.4.0.5）。")
            .RegisterEntry("资源记录器中添加了加载资源的操作系统线程 ID（1.4.0.5）。")
            .RegisterEntry("在设置标签页中添加了链接到我（Ottermandias）的 Ko-Fi 和 Patreon 的按钮。欢迎但不强制使用！:D ")
            .RegisterHighlight("模组设置组合框现在支持使用 Ctrl 键进行鼠标滚轮滚动并具有筛选功能（1.4.0.4）。")
            .RegisterEntry("使用鼠标中键切换设计现在可以正确配合临时设置工作（1.4.0.4）。")
            .RegisterEntry("更新了一些 BNPC 关联（1.4.0.3）。")
            .RegisterEntry("修复了形态和属性的进一步问题（1.4.0.4）。")
            .RegisterEntry("Penumbra 现在可以处理导入时被 TexTools 破坏的 MipMap 偏移纹理，并移除不必要的 MipMaps（1.4.0.3）。")
            .RegisterEntry("为新的群组类型更新了模组合并器（1.4.0.3）。")
            .RegisterEntry("添加了通过 IPC 查询 Penumbra 支持功能的接口（1.4.0.3）。")
            .RegisterEntry("形态名称现在可以在 Penumbra 的模型编辑器中编辑（1.4.0.2）。")
            .RegisterEntry("属性和形态现在可以完全切换（1.4.0.2）。")
            .RegisterEntry("修复了属性和形态的几个问题（1.4.0.1）。");
