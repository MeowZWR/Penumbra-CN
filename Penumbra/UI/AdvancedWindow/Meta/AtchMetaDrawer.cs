...
public sealed class AtchMetaDrawer : MetaDrawer<AtchIdentifier, AtchEntry>
{
    ...
    public void ImportFile(string filePath)
    {
        ...
        {
            Penumbra.Messager.AddMessage(new Notification(ex, "导入的.atch文件不包含种族代码（cXXXX）。",
                "无法导入.atch文件：", NotificationType.Warning));
        }
        catch (Exception ex)
        {
            Penumbra.Messager.AddMessage(new Notification(ex, "无法导入.atch文件。", "无法导入.atch文件：",
    ...
    private static bool DrawEntry(in AtchEntry defaultEntry, ref AtchEntry entry, bool disabled)
    {
        ...
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "骨骼名称"u8);

        Im.Item.SetNextWidthScaled(200);
        changes |= Im.Input.Scalar("##AtchScale"u8, ref entry.Scale);
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "缩放"u8);

        Im.Table.NextColumn();
        Im.Item.SetNextWidthScaled(120);
        changes |= Im.Input.Scalar("##AtchOffsetX"u8, ref entry.OffsetX);
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "偏移 X 坐标"u8);
        Im.Item.SetNextWidthScaled(120);
        changes |= Im.Input.Scalar("##AtchRotationX"u8, ref entry.RotationX);
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "旋转 X 轴"u8);

        Im.Table.NextColumn();
        Im.Item.SetNextWidthScaled(120);
        changes |= Im.Input.Scalar("##AtchOffsetY"u8, ref entry.OffsetY);
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "偏移 Y 坐标"u8);
        Im.Item.SetNextWidthScaled(120);
        changes |= Im.Input.Scalar("##AtchRotationY"u8, ref entry.RotationY);
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "旋转 Y 轴"u8);

        Im.Table.NextColumn();
        Im.Item.SetNextWidthScaled(120);
        changes |= Im.Input.Scalar("##AtchOffsetZ"u8, ref entry.OffsetZ);
        Im.Tooltip.OnHover("偏移 Z 坐标"u8);
        Im.Item.SetNextWidthScaled(120);
        changes |= Im.Input.Scalar("##AtchRotationZ"u8, ref entry.RotationZ);
        Im.Tooltip.OnHover(HoveredFlags.AllowWhenDisabled, "旋转 Z 轴"u8);
    ...
    private static bool DrawGender(ref AtchIdentifier identifier, bool disabled)
    {
        ...
        if (!ImEx.Icon.Button(isMale ? FontAwesomeIcon.Mars.Icon() : FontAwesomeIcon.Venus.Icon(), "性别"u8,
