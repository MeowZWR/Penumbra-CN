    ...
    : IDisposable
{
    ...
    private void RedrawOnSaveBox()
    {
        ...
        Im.Tooltip.OnHover("每当你在这里保存文件时，强制重新绘制你的玩家角色。"u8);
    }
    ...
    private void DefaultInput()
    {
        ...
        Im.Input.Text("##defaultInput"u8, ref _defaultPath, "输入游戏路径来比对..."u8, maxLength: Utf8GamePath.MaxGamePathLength);
        ...
        Im.Line.Same();
        if (ImEx.Icon.Button(LunaStyle.SaveIcon, "导出此文件。"u8, _defaultFile is null))
        ...
        Im.Line.Same();
        if (ImEx.Icon.Button(LunaStyle.ImportIcon, $"添加此文件的副本到 {_quickImport.OptionName}。", !_quickImport.CanExecute))
        {
            try
            {
    ...
    private void DrawFileSelectCombo()
    {
        ...
        {
            if (_combo.Draw("##select"u8, $"选择 {fileType} 文件...", StringU8.Empty, Im.ContentRegion.Available.X, out var newSelection))
                UpdateCurrentFile(newSelection);
        }
    }
    ...
    private void SaveButton()
    {
        ...
        if (ImEx.Button("保存到文件"u8, Vector2.Zero,
    ...
    {
        if (ImEx.Button("重置修改"u8, Vector2.Zero,
