    ...
    )
{
    protected override void PreDraw(in Cache cache)
    {
        ...
        if (ImEx.LogarithmicInput("忽略小于此大小的纹理"u8, FormattingFunctions.HumanReadableSize(config.LowerTextureSizeLimit),
                config.LowerTextureSizeLimit, out var newLowerLimit))
            config.LowerTextureSizeLimit = newLowerLimit;
        Im.Item.SetNextWidthScaled(100);
        if (ImEx.LogarithmicInput("忽略较小分辨率的纹理"u8, config.SmallTextureDimensionLimit, out var newSmallLimit, 0))
            config.SmallTextureDimensionLimit = newSmallLimit;
        Im.Item.SetNextWidthScaled(100);
        if (ImEx.LogarithmicInput("显示较大分辨率的纹理，即使已被压缩"u8, config.LargeTextureDimensionLimit,
                out var newLargeLimit, 0))
            config.LargeTextureDimensionLimit = newLargeLimit;
        cache.DrawScanButtons();
        LunaStyle.DrawSeparator();
        if (Im.Checkbox("在执行破坏性操作前创建备份"u8, config.CreateTextureBackups))
            config.CreateTextureBackups ^= true;
        Im.Tooltip.OnHover(
            "启用此选项后，任何纹理在被覆盖之前会添加 '.bak' 后缀，并在其所处路径下覆盖以前同名的备份文件。"u8);
        Im.Item.SetNextWidthScaled(100);
        if (ImEx.LogarithmicInput("纹理分辨率限制"u8, config.TextureDimensionLimit, out var newDimensionLimit, 4))
            config.TextureDimensionLimit = newDimensionLimit;
        Im.Tooltip.OnHover(
            "自动调整分辨率时纹理分辨率在两个方向上的上限。纹理分辨率将在两个方向上减半，直到两个方向都小于或等于此值。"u8);
    }
        ...
        private void DrawSolidColorButton(in TextureOptimizationCacheObject item, int globalIndex)
        {
            ...
            if (Im.Item.Hovered(HoveredFlags.AllowWhenDisabled))
            {
                ...
                if (_optimization.CanReplaceWithSwap(item.ScannedObject.FilePath, mod, color, out var path))
                    Im.Text($"将此文件的所有引用替换为文件交换 {path} 并删除该文件。");
                else
                    Im.Text($"将此文件替换为颜色为 {color} 的 1x1 纹理。");
                if (!active)
                    Im.Text($"\n按住 {LunaStyle.Modifier.Misclick} 键以替换。");
            }
        }
        ...
        private void DrawCompressionButton(in TextureOptimizationCacheObject item, int globalIndex)
        {
            ...
            if (item.CompressionTask is null)
            {
                ...
                if (Im.Item.Hovered(HoveredFlags.AllowWhenDisabled))
                {
                    ...
                    Im.Text($"使用块压缩 {usage} 压缩此纹理。");
                    if (!active)
                        Im.Text($"\n按住 {LunaStyle.Modifier.Destructive} 键以压缩。");
                    if (otherTask)
                        Im.Text("\n等待调整分辨率任务完成。"u8);
                }
            }
            else if (item.CompressionTask.IsFaulted)
            {
                ...
                if (Im.Item.Hovered(HoveredFlags.AllowWhenDisabled))
                {
                    ...
                    if (!active)
                        Im.Text($"\n按住 {LunaStyle.Modifier.Destructive} 键以重试。");
                    if (otherTask)
                        Im.Text("\n等待调整分辨率任务完成。"u8);
                }
            }
        ...
        private void DrawRestrictDimensionsButton(in TextureOptimizationCacheObject item, int globalIndex)
        {
            ...
            if (item.ResizeTask is null)
            {
                ...
                if (Im.Item.Hovered(HoveredFlags.AllowWhenDisabled))
                {
                    ...
                    if (!active)
                        Im.Text($"\n按住 {LunaStyle.Modifier.Destructive} 键以调整分辨率。");
                }
            }
