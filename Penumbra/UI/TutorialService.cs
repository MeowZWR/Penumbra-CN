...
public class TutorialService : IUiService
{
    ...
    public TutorialService(EphemeralConfig config)
    {
        ...
        _tutorial = new Tutorial()
            ...
            .Register("初始设置，步骤3：合集", "合集是您已安装模组的设置列表。\n\n"
              + "这是我们的下一站！\n\n"
              + "设置根目录后，点击此选项卡继续教程！")
            .Register("初始设置，步骤4: 管理合集",
                "在左边，我们有合集选择器。在这里我们可以创建新的合集 - 创建空白合集或者复制现有的合集 - 并删除任何不再需要的合集。\n"
              + $"名为'{ModCollectionIdentity.DefaultCollectionName}'的合集不能被删除。")
            .Register($"初始设置，步骤5: {SelectedCollection}",
                $"在选择器中高亮显示的合集是'{SelectedCollection}'。这是我们正在查看和操作的合集。\n我们稍后在下一个选项卡中对模组做的任何修改都将应用到此合集。\n"
              + $"我们应该已经选中了合集'{ModCollectionIdentity.DefaultCollectionName}'，我们现在只是做基础设置，还不需要对它做任何操作。\n\n")
            .Register("初始设置，步骤6: 简单分配",
                "除了用合集来管理不同的模组设置，我们还可以使用分配功能来分配合集，可以让不同的模组设置生效于不同的角色。\n"
              + "简单分配面板提供对大部分人来说按照说明就足够使用的分配功能。\n"
              + $"如果你是初次使用，你可以看到合集'{ModCollectionIdentity.DefaultCollectionName}'已经分配给了'{CollectionType.Default.ToName()}'和'{CollectionType.Interface.ToName()}'。\n"
              + "您还可以通过点击功能按钮，指定使用‘不使用任何模组’来替代合集。")
            .Register("独立分配",
            ...
            .Register("合集详情",
              ...
              + "继承可以用于使一个合集采用另一个合集的设置，只要它本身未设置相关的模组。")
            .Register("匿名模式",
