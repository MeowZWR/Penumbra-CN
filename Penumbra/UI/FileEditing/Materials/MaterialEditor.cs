...
public sealed partial class MaterialEditor : IFileEditor
{
    ...
    private bool DrawVersionUpdate(bool disabled)
    {
        ...
        if (!ImEx.Button("将 MTRL 版本更新至「金曦之遗辉」"u8, Colors.PressEnterWarningBg, default, Im.ContentRegion.Available with { Y = 0 },
                "如果材质无法加载或应该使用旧版着色器，可尝试使用此方法。\n\n此操作不可逆。"u8))
            return false;
    ...
    private bool DrawBackFaceAndTransparency(bool disabled)
    {
        ...
        {
            if (Im.Checkbox("启用透明度"u8, ref tmp))
            {
        ...
        if (_shpkLoading)
        {
            ...
            Im.Text("正在加载着色器 (.shpk) 文件。部分功能将在加载完成后才可用。"u8,
