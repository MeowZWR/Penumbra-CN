...
public partial class MaterialEditor
{
    ...
    private bool DrawShaderSection(bool disabled)
    {
        ...
        if (!_shpkLoading && (_associatedShpk is null || _associatedShpkDevkit is null))
        {
            ...
            if (_associatedShpk is null)
                Im.Text("无法找到适合的着色器 (.shpk) 文件进行交叉引用。一些功能将会缺失。"u8,
    ...
    private bool DrawPackageNameInput(bool disabled)
    {
        ...
        using var c = Im.Combo.Begin("着色器包"u8, Mtrl.ShaderPackage.Name);
        if (c)
    ...
    private bool DrawShaderFlagsInput(bool disabled)
    {
        ...
        Im.Item.SetNextWidthScaled(250.0f);
        if (!Im.Input.Scalar("着色器标志"u8, ref shpkFlags, "%08X"u8,
    ...
    private bool DrawAdditionalFlagsInput(bool disabled)
    {
        ...
        Im.Item.SetNextWidthScaled(250.0f);
        if (!Im.Input.Scalar("附加标志"u8, ref additionalFlags, "%08X"u8,
                flags: InputTextFlags.CharsHexadecimal | (disabled ? InputTextFlags.ReadOnly : InputTextFlags.None)))
            return false;
    ...
    {
        var tooltip = "点击复制文件路径到剪贴板"u8;

        Im.Dummy(new Vector2(Im.Style.TextHeight / 2));
        ImEx.CopyOnClickSelectable(_associatedShpk is null ? "关联的 .shpk 文件：无" : $"关联的 .shpk 文件：{_loadedShpkPathName}",
            _loadedShpkPathName, tooltip);
        ImEx.CopyOnClickSelectable(
            _associatedShpkDevkit is null ? "关联的开发工具包文件：无" : $"关联的开发工具包文件：{_loadedShpkDevkitPathName}",
            _loadedShpkDevkitPathName, tooltip);
        ImEx.CopyOnClickSelectable(
            _associatedBaseDevkit is null ? "基础开发工具包文件：无" : $"基础开发工具包文件：{_loadedBaseDevkitPathName}",
            _loadedBaseDevkitPathName, tooltip);

        if (Im.Button("关联自定义 .shpk 文件"u8))
        ...
        Im.Line.Same();
        if (ImEx.Button("关联默认 .shpk 文件"u8, Vector2.Zero, moddedPath.ToPath(), moddedPath.Equals(_loadedShpkPath)))
            LoadShpk(moddedPath);

        if (!gamePath.Path.Equals(moddedPath.InternalName))
        {
            Im.Line.Same();
            if (ImEx.Button("关联未修改的 .shpk 文件"u8, Vector2.Zero, defaultPath, gamePath.Path.Equals(_loadedShpkPath.InternalName)))
    ...
    private void DrawMaterialShaders()
    {
        ...
        using (var node = Im.Tree.Node("候选着色器"u8))
        {
            if (node)
