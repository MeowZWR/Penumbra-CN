...
public partial class MaterialEditor
{
    ...
    private bool DrawRowHeader(int rowIdx, bool disabled)
    {
        ...
        CenteredTextInRest($"行 {(rowIdx >> 1) + 1}{"AB"[rowIdx & 1]}");
    ...
    private bool DrawTemplate(ColorTable table, ColorDyeTable? dyeTable, in DyePack? dyePack, int rowIdx)
    {
        ...
        ret |= CtShaderIdPicker("着色器 ID"u8, default, row.ShaderId, v => table[rowIdx].ShaderId = v);

        Im.Line.Same(subColWidth);
        Im.Item.SetNextWidth(scalarSize);
        ret |= CtDragScalar("滚动参数"u8, default, (ushort)row.Scalar23, "%d"u8, (ushort)0, (ushort)2, 0.25f,
    ...
    private static bool DrawExtra(ColorTable table, ColorDyeTable? dyeTable, in DyePack? dyePack, int rowIdx)
    {
        ...
        ret |= CtDragScalar("曝光值"u8, default, exposureValue, rawExposureValue is 0.0f ? "-∞"u8 : "%.1f"u8, ExposureMinInfDummy,
            ExposureMax,                        0.025f,
        ...
        {
            Im.Line.Same(dyeOffset);
            ret |= CtApplyStainCheckbox("##dyeExposure"u8, "在染色时应用曝光值"u8, dye.Exposure,
            ...
            CtDragScalar("##dyePreviewExposure"u8, "曝光值的染色预览"u8, dyeExposureValue,
                dyePack?.Exposure == Half.Zero ? "-∞"u8 : "%.1f"u8);
        }

        Im.Line.Same(subColWidth);
        Im.Item.SetNextWidth(scalarSize);
        ret |= CtDragHalf(isRowB ? "各向异性度（未使用）"u8 : "各向异性度"u8, default, row.Anisotropy, "%.1f"u8, 0.0f,
            HalfMaxValue,                                                           0.025f,  v => table[rowIdx].Anisotropy = v);
        if (dyeTable is not null)
        {
            Im.Line.Same(subColWidth + dyeOffset);
            ret |= CtApplyStainCheckbox("##dyeAnisotropy"u8, "在染色时应用各向异性度"u8, dye.Anisotropy,
                b => dyeTable[rowIdx].Anisotropy = b);
            Im.Line.SameInner();
            Im.Item.SetNextWidth(scalarSize);
            CtDragHalf("##dyePreviewAnisotropy"u8, "各向异性度的染色预览"u8, dyePack?.Anisotropy, "%.1f"u8);
        }
    ...
    private bool DrawDye(ColorDyeTable dyeTable, in DyePack? dyePack, int rowIdx)
    {
        ...
        Im.Item.SetNextWidth(scalarSize);
        ret |= CtDragScalar("染色通道"u8, default, dye.Channel + 1, "%d"u8, 1, StainService.GetUiChannelCount(_config.Editing), 0.1f,
