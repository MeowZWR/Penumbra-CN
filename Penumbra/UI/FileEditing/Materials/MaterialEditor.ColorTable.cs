...
public partial class MaterialEditor
{
    ...
    private static bool DrawBlending(ColorTable table, ColorDyeTable? dyeTable, DyePack? dyePack, int rowIdx)
    {
        ...
        if (dyeTable is not null)
        {
            ...
            Im.Line.SameInner();
            Im.Item.SetNextWidth(scalarSize);
            CtDragHalf("##dyePreviewAnisotropy"u8, isRowB ? "Field #19 的染色预览"u8 : "各向异性度的染色预览"u8,
    ...
    private static bool DrawFurther(ColorTable table, ColorDyeTable? dyeTable, DyePack? dyePack, int rowIdx)
    {
        ...
        {
            Im.Line.Same(dyeOffset);
            ret |= CtApplyStainCheckbox("##dyeScalar11"u8, "在染色时应用 Field #11"u8, dye.Scalar3,
                b => dyeTable[rowIdx].Scalar3 = b);
            Im.Line.SameInner();
            Im.Item.SetNextWidth(scalarSize);
            CtDragHalf("##dyePreviewScalar11"u8, "Field #11 的染色预览"u8, dyePack?.Scalar3, "%.2f"u8);
        }
