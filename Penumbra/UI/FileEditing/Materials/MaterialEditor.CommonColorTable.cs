...
public partial class MaterialEditor
{
    ...
    private bool ColorTableHeaderButton(bool disabled)
    {
        ...
        var headerWidth = Im.ContentRegion.Available.X
          + sideShift
          - ImEx.Icon.CalculateLabeledButtonSize(LunaStyle.OnHoverIcon, "全部高亮"u8).X;
        ...
        ImEx.Icon.LabeledButton(LunaStyle.OnHoverIcon, "全部高亮###switchToEquipBar"u8,
            "尽可能在角色上高亮整张颜色表。\n\n高亮颜色可在 Penumbra 设置中配置。"u8,
        ...
        return Im.Tree.Header("颜色表"u8, TreeNodeFlags.DefaultOpen);
    }
    ...
    private void ColorTableCopyAllClipboardButton()
    {
        ...
        if (!Im.Button("将所有行导出到剪贴板"u8, ImEx.ScaledVector(200, 0)))
            return;
    ...
    private bool ColorTablePasteAllClipboardButton(bool disabled)
    {
        ...
        if (!ImEx.Button("从剪贴板导入所有行"u8, ImEx.ScaledVector(200, 0), disabled))
            return false;
    ...
    private bool ColorTableDyeableCheckbox()
    {
        ...
        var ret     = Im.Checkbox("可染色的"u8, ref dyeable);
