...
public partial class CombinedTexture
{
    ...
    public void DrawMatrixInputRight(float width)
    {
        ...
        ret |= _combineCombo.Draw("合并操作"u8, ref _combineOp, StringU8.Empty, 250 * Im.Style.GlobalScale);
        var resizeOp = GetActualResizeOp(_resizeOp, _combineOp);
        using (Im.Disabled((int)resizeOp < 0))
        {
            ret |= _resizeCombo.Draw("调整大小模式"u8, ref _resizeOp, StringU8.Empty, 250 * Im.Style.GlobalScale);
        }
