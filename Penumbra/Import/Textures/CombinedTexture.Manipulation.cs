...
public partial class CombinedTexture
{
    ...
    public void DrawMatrixInputRight(float width)
    {
        ...
        Im.Line.Same();
        Im.Item.SetNextWidthScaled(75);
        Im.Drag("偏移##YOffset"u8, ref _offsetY, speed: 0.5f);
        ret |= Im.Item.DeactivatedAfterEdit;

        Im.Item.SetNextWidthScaled(200);
        ret |= _combineCombo.Draw("合并操作"u8, ref _combineOp, StringU8.Empty, 200 * Im.Style.GlobalScale);
        var resizeOp = GetActualResizeOp(_resizeOp, _combineOp);
        using (Im.Disabled((int)resizeOp < 0))
        {
            ret |= _resizeCombo.Draw("调整大小模式"u8, ref _resizeOp, StringU8.Empty, 200 * Im.Style.GlobalScale);
        }
    ...
    {
        using var combo = Im.Combo.Begin("预设"u8, StringU8.Empty, ComboFlags.NoPreview);
        if (!combo)
            return false;
