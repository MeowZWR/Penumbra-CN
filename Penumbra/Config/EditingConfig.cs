...
public sealed partial class EditingConfig : ConfigurationFile<FilenameService>
{
    ...
    [ConfigProperty]
    private bool _hideDdsFiles = true;

    [ConfigProperty]
    private bool _hidePngFiles = true;

    [ConfigProperty]
    private bool _hideJpegFiles = true;

    [ConfigProperty]
    private bool _hideJsonFiles = true;

    [ConfigProperty]
    private bool _hideTgaFiles = true;

    [ConfigProperty]
    private bool _hideBmpFiles = true;

    [ConfigProperty]
    private bool _hideGifFiles = true;

    [ConfigProperty]
    private bool _hideTiffFiles = true;

    [ConfigProperty]
    private bool _hideWebpFiles = true;

    [ConfigProperty]
    private bool _hideXcpFiles = true;
    ...
    protected override void AddData(Utf8JsonWriter j)
    {
        ...
        using (var tempObject = j.TemporaryObject("FileFilters"u8))
        {
            tempObject.WriteIfNot("HideDdsFiles"u8,  HideDdsFiles,  true);
            tempObject.WriteIfNot("HidePngFiles"u8,  HidePngFiles,  true);
            tempObject.WriteIfNot("HideJpegFiles"u8, HideJpegFiles, true);
            tempObject.WriteIfNot("HideJsonFiles"u8, HideJsonFiles, true);
            tempObject.WriteIfNot("HideTgaFiles"u8,  HideTgaFiles,  true);
            tempObject.WriteIfNot("HideBmpFiles"u8,  HideBmpFiles,  true);
            tempObject.WriteIfNot("HideGifFiles"u8,  HideGifFiles,  true);
            tempObject.WriteIfNot("HideTiffFiles"u8, HideTiffFiles, true);
            tempObject.WriteIfNot("HideWebpFiles"u8, HideWebpFiles, true);
            tempObject.WriteIfNot("HideXcpFiles"u8,  HideXcpFiles,  true);
        }
    ...
    protected override void LoadData(in JsonElement j)
    {
        ...
        if (j.TryReadObject("FileFilters"u8, out var fileFilters))
        {
            HideDdsFiles  = fileFilters.PropertyOrDefault("HideDdsFiles"u8,  HideDdsFiles);
            HidePngFiles  = fileFilters.PropertyOrDefault("HidePngFiles"u8,  HidePngFiles);
            HideJpegFiles = fileFilters.PropertyOrDefault("HideJpegFiles"u8, HideJpegFiles);
            HideJsonFiles = fileFilters.PropertyOrDefault("HideJsonFiles"u8, HideJsonFiles);
            HideTgaFiles  = fileFilters.PropertyOrDefault("HideTgaFiles"u8,  HideTgaFiles);
            HideBmpFiles  = fileFilters.PropertyOrDefault("HideBmpFiles"u8,  HideBmpFiles);
            HideGifFiles  = fileFilters.PropertyOrDefault("HideGifFiles"u8,  HideGifFiles);
            HideTiffFiles = fileFilters.PropertyOrDefault("HideTiffFiles"u8, HideTiffFiles);
            HideWebpFiles = fileFilters.PropertyOrDefault("HideWebpFiles"u8, HideWebpFiles);
            HideXcpFiles  = fileFilters.PropertyOrDefault("HideXcpFiles"u8,  HideXcpFiles);
        }
